...
select * from bajajauto limit 10;

# convert Date column values from string to date format
UPDATE bajajauto SET Date = STR_TO_DATE(Date,'%d-%M-%Y');

# convert data type of Date column to date
ALTER TABLE bajajauto MODIFY COLUMN Date date;

desc bajajauto;

# Check if 'Date' column has unique values
SELECT COUNT(DISTINCT date) AS 'Number of Distinct Dates', COUNT(date) AS 'Number of Rows' FROM bajajauto;

# Making Date column as a primary key for the table bajaj
ALTER TABLE bajajauto ADD PRIMARY KEY (Date);
...
select * from eichermoters limit 10;

# convert Date column values from string to date format
UPDATE eichermoters SET Date = STR_TO_DATE(Date,'%d-%M-%Y');

# convert data type of Date column to date
ALTER TABLE eichermoters MODIFY COLUMN Date date;

desc eichermoters;

# Check if 'Date' column has unique values
SELECT COUNT(DISTINCT date) AS 'Number of Distinct Dates', COUNT(date) AS 'Number of Rows' FROM eichermoters;

# Making Date column as a primary key for the table eicher
ALTER TABLE eichermoters ADD PRIMARY KEY (Date);
...
select * from heromotocorp limit 10;

# convert Date column values from string to date format
UPDATE heromotocorp SET Date = STR_TO_DATE(Date,'%d-%M-%Y');

# convert data type of Date column to date
ALTER TABLE heromotocorp MODIFY COLUMN Date date;

desc heromotocorp;

# Check if 'Date' column has unique values
SELECT COUNT(DISTINCT date) AS 'Number of Distinct Dates', COUNT(date) AS 'Number of Rows' FROM heromotocorp;

# Making Date column as a primary key for the table hero
ALTER TABLE heromotocorp ADD PRIMARY KEY (Date);
...
SELECT b.Date AS Date, b.`Close Price` as Bajajauto, tc.`Close Price` as TCS, tv.`Close Price` as TVS,
i.`Close Price` as Infosys, e.`Close Price` as Eichermotors, h.`Close Price` as Heromotocorp
